...
Vous pouvez utiliser cette rubrique pour savoir comment configurer le client virtuel passerelles, à l’aide des applets de commande Windows PowerShell et des scripts, pour fournir des réseaux virtuels de vos clients avec une connectivité de site à leurs sites d’entreprise et à Internet.   
  
Cette passerelle prend en charge des clients jusqu'à cent, en fonction de la bande passante utilisée par chaque client. Contrôleur de réseau vous permet d’ajouter le client virtuel passerelles aux instances de cette passerelle qui sont membres des pools de passerelle. Contrôleur de réseau détermine automatiquement la meilleure passerelle RAS à utiliser lorsque vous déployez une passerelle virtuelle pour vos clients.  
  
Chaque passerelle virtuelle correspond à un client particulier et se compose d’un ou plusieurs connexions réseau (tunnels VPN de site à site) et, éventuellement, des connexions de protocole BGP (Border Gateway). Cela permet à vos clients à se connecter leurs clients réseau virtuel à un réseau externe, par exemple, un réseau d’entreprise du client, un réseau de fournisseur de service ou à internet.  
  
Lorsque vous déployez une passerelle virtuelle locataire, vous disposez des options de configuration suivantes:  
  
**Options de connexion réseau**  
- Réseau privé virtuel de site à site de IPSec (VPN)   
- Encapsulation générique de routage (GRE)   
- Transfert de couche 3  
  
**Options de configuration BGP**  
- Configuration du routeur BGP  
- Configuration d’homologue BGP  
- Configuration des stratégies de routage BGP  
  
Les exemples de scripts Windows PowerShell et les commandes dans cette rubrique expliquent comment déployer une passerelle virtuelle locataire sur une passerelle avec chacune de ces options.  
  
Cette rubrique contient les sections suivantes.  
  
- [Ajouter une passerelle virtuelle pour un client](#bkmk_addgwy)  
- [Ajouter une connexion de réseau VPN site à site pour un client (GRE, IPsec ou L3)](#bkmk_s2s1)  
- [Configurer la passerelle comme un routeur BGP](#bkmk_bgp1)  
- [Configurer une passerelle avec tous les types de connexion trois (L3 IPsec, GRE,) et BGP](#bkmk_all3)  
- [Modifier ou supprimer une passerelle d’un réseau virtuel](#bkmk_modify)  
  
   
>[!IMPORTANT]  
>Avant d’exécuter toutes les exemples de commandes Windows PowerShell et les scripts fournis dans cette rubrique, vous devez modifier toutes les valeurs des variables afin que les valeurs sont appropriées pour votre déploiement.  
  
## <a name="bkmk_addgwy"></a>Ajouter une passerelle virtuelle pour un client  
  
Étape1: Vérifiez que l’objet de Pool passerelle existe dans le contrôleur de réseau.  
```  
...
```  
Étape2: Vérifier que le sous-réseau à utiliser pour le routage des paquets de réseau virtuel du client existe dans le contrôleur de réseau; et récupérez le sous-réseau virtuel qui doit être utilisé pour le routage entre la passerelle du client et le réseau virtuel.  
...
```  
Étape3: Créer une passerelle virtuelle objet JSON et l’ajouter à un contrôleur de réseau.  
...
## <a name="bkmk_s2s1"></a>Ajouter une connexion de réseau VPN site à site pour un client (GRE, IPsec ou L3)  
  
Vous pouvez créer une connexion VPN site à site avec IPsec, GRE ou de couche 3 (L3) en utilisant les exemples suivants pour chaque type de passerelle de transfert.  
  
### <a name="ipsec-vpn-site-to-site-network-connection"></a>Connexion de réseau IPsec VPN site à site  
  
Créer un objet JSON de connexion réseau et l’ajouter à un contrôleur de réseau.  
...
```  
### <a name="gre-vpn-site-to-site-network-connection"></a>Connexion de réseau GRE VPN site à site  
...
```  
### <a name="l3-forwarding-network-connection"></a>L3 Transfert connexion réseau  
  
Pour configurer une connexion réseau de transfert L3, vous devez également configurer un réseau logique correspondant.   
  
Étape1: Configurer un réseau logique pour le L3 transfert de connexion réseau.  
...
```  
Étape2: Créer un objet JSON de connexion réseau et l’ajouter au contrôleur de réseau.  
...
## <a name="bkmk_bgp1"></a>Configurer la passerelle comme un routeur BGP  
  
Vous pouvez utiliser les exemples de scripts suivants pour configurer la passerelle comme un routeur de protocole BGP (Border Gateway).  
  
### <a name="add-a-bgp-router-for-the-tenant"></a>Ajouter un routeur BGP pour le client  
  
Créer un objet JSON du routeur BGP et ajoutez-le à un contrôleur de réseau.  
...
```  
### <a name="add-a-bgp-peer-for-this-tenant-corresponding-to-the-site-to-site-vpn-network-connection-added-above"></a>Ajouter un homologue BGP pour ce client, correspondant à la connexion au réseau VPN site à site ajouté ci-dessus  
  
Créer un objet JSON d’homologue BGP et l’ajouter à un contrôleur de réseau.  
...
```  
## <a name="bkmk_all3"></a>Configurer une passerelle avec tous les types de connexion trois (L3 IPsec, GRE,) et BGP  
Si vous le souhaitez, vous pouvez combiner toutes les étapes précédentes et configurer une passerelle virtuelle locataire avec toutes les options de connexion:   
...
```  
## <a name="bkmk_modify"></a>Modifier ou supprimer une passerelle d’un réseau virtuel  
  
Vous pouvez utiliser les exemples de scripts suivants pour modifier ou supprimer une passerelle existante.  
  
### <a name="modify-the-configuration-of-an-existing-gateway"></a>Modifier la configuration d’une passerelle existante  
Vous pouvez utiliser les commandes suivantes pour modifier une passerelle existante.  
  
Étape1: Récupérer la configuration du composant et le stocker dans une variable  
...
```  
Étape2: Accédez à la structure de variable pour atteindre la propriété requise et affectez-lui la valeur de mises à jour  
  
```  
$nwConnection.properties.IpSecConfiguration.SharedSecret = "C0mplexP@ssW0rd"  
```  
Étape3: Ajouter la configuration modifiée pour remplacer l’ancienne configuration sur le contrôleur de réseau  
...
Vous pouvez utiliser les commandes Windows PowerShell suivantes pour supprimer les fonctionnalités de la passerelle individuelle ou l’ensemble de la passerelle.  
  
#### <a name="remove-a-network-connection"></a>Supprimer une connexion réseau  
```  
...
```  
#### <a name="remove-a-bgp-peer"></a>Supprimer un homologue BGP  
```  
...
```  
#### <a name="remove-a-bgp-router"></a>Supprimer un routeur BGP  
```  

...
|   `{n,m}`   |                                                Correspond à au moins n et au plus m fois \(m et n sont des entiers négatifs non\-\).                                                |                               `/o{1,3}/ matches the first three instances of the letter o in "fooooood."`                               |
|   `[xyz]`   |                                                       Correspond à l’un des caractères entourés \(un jeu de caractères\).                                                        |                                                  `/[abc]/ matches the "a" in "plain."`                                                  |
...
|    `\s`     |                                   Correspond à n’importe quel caractère d’espace blanc, y compris l’espace, la tabulation et le flux de formulaire \(équivalent à `[ \f\n\r\t\v]`\).                                   |                                                                 &nbsp;                                                                  |
...
|    `\W`     |                                           Correspond à n’importe quel caractère de mot non\-, à l’exclusion du trait de soulignement \(équivalent à `[^A-Za-z0-9_]`\).                                           |                                                                 &nbsp;                                                                  |
|   `\num`    | Fait référence aux correspondances mémorisées \(`?num`, où num est un entier positif\).  Cette option peut être utilisée uniquement dans la zone de texte **remplacer** lors de la configuration de la manipulation d’attributs. |                                       `\1` remplace ce qui est stocké dans la première correspondance mémorisée.                                       |
|   `/n/ `    |                      Autorise l’insertion de codes ASCII dans des expressions régulières \(`?n`, où n est une valeur d’échappement octale, hexadécimale ou décimale\).                       |                                                                 &nbsp;                                                                  |

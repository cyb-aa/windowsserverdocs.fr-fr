...
### <a name="evaluating"></a>Évalués
Cette section est moins adaptée à l’évaluation des demandes relatives au trafic de réplication, qui se concentre sur le trafic qui traverse le WAN et qui est traitée en détail dans [Active Directory le trafic](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-2000-server/bb742457(v=technet.10))de réplication, par rapport à l’évaluation de la bande passante totale et du réseau. la capacité nécessaire, les requêtes du client, les stratégie de groupe les applications, etc. Pour les environnements existants, vous pouvez les collecter à l’aide des compteurs de performance «\*interface réseau () \Octets reçus/s» et «interface(\*)réseau  \Octets envoyés/s». Intervalles d’échantillonnage des compteurs d’interface réseau en 15, 30 ou 60 minutes. Tout ce qui est moins est généralement trop volatil pour les mesures appropriées; tout ce qui est plus important lissera les aperçus quotidiens de manière excessive.

> [!NOTE]
> En règle générale, la majeure partie du trafic réseau sur un contrôleur de réseau est sortante, car le contrôleur de réseau répond aux requêtes du client. Il s’agit de la raison pour laquelle le trafic sortant est concentré, bien qu’il soit recommandé d’évaluer chaque environnement pour le trafic entrant également. Les mêmes approches peuvent être utilisées pour résoudre et examiner les exigences en matière de trafic réseau entrant. Pour plus d’informations, consultez l’article [929851 de la base de connaissances: La plage de ports dynamiques par défaut pour TCP/IP a changé dans Windows Vista et dans Windows](http://support.microsoft.com/kb/929851)Server 2008.
...
- **Pour les environnements &ndash; existants** , la section intitulée «pour activer l’enregistrement de l’espace disque libéré par la défragmentation» dans l’article [limites de stockage](https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-2000-server/cc961769(v=technet.10)).
- **Pour les nouveaux &ndash; environnements** , l’article a intitulé [estimations de croissance pour les utilisateurs Active Directory et les unités d’organisation](https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-2000-server/cc961779(v=technet.10)).
...
- \*Disque logique disque \Avg s/lecture (par exemple, si NTDS. dit est stocké sur le lecteur D:/ le chemin d’accès complet est disque logique (D:) \Avg disque s/lecture)
- Disque logique\*() \Avg disque s/écriture
- \*Disque logique disque \Avg s/transfert
- Disque logique\*() \ lectures/s
- LogicalDisk (\*) \ écritures/s
- LogicalDisk (\*) \ transferts/s
...
>  
> - Disque logique *\<(\>NTDS*) \Avg s/lecture indique si le stockage actuel est correctement dimensionné.  Si les résultats sont à peu près égaux au temps d’accès au disque pour le type de disque, LogicalDisk ( *\<NTDS\>* ) \ lectures/s est une mesure valide.  Vérifiez les spécifications du fabricant pour le stockage sur le back end, mais les plages correctes pour LogicalDisk ( *\<NTDS\>* ) \Avg disque s/lecture seraient à peu près:
...
> - LogicalDisk ( *\<NTDS\>* ) \ lectures/s correspond à la quantité d’e/s en cours d’exécution.
>   - Si LogicalDisk ( *\<NTDS\>* ) \Avg disque s/lecture se trouve dans la plage optimale pour le stockage principal, le disque logique ( *\<NTDS\>* ) \ lectures/s peut être utilisé directement pour dimensionner le stockage.
>   - Si LogicalDisk ( *\<NTDS\>* ) \Avg disque s/lecture ne se trouve pas dans la plage optimale pour le stockage principal, des e/s supplémentaires sont nécessaires en fonction de la formule suivante:
>     > (LogicalDisk ( *\<NTDS\>* ) \Avg disque s/lecture) &divide; (temps d’accès au disque du &times; support physique) (disque logique ( *\<NTDS\>* ) \Avg s/lecture)
...
De même, du point de vue de l’invité, comme il existe des chemins de code supplémentaires qui doivent être parcourus, il y a un impact sur les performances de l’utilisation d’un hôte pour accéder à n’importe quel stockage. Le test des performances de stockage indique que la virtualisation a un impact sur le débit de façon subjective de l’utilisation du processeur du système hôte (Voir l’annexe A: Critères de dimensionnement de l’UC), qui est évidemment influencé par les ressources de l’hôte demandées par l’invité. Cela contribue aux considérations relatives à la virtualisation en ce qui concerne le traitement des besoins dans un scénario virtualisé (consultez [Considérations sur la virtualisation pour le traitement](#virtualization-considerations-for-processing)).
...
- Étant donné que chaque système est d’environ 40% et que tous les systèmes ont le même nombre de processeurs, en cas d’échec ou de mise hors connexion, les systèmes restants s’exécutaient à une valeur estimée de 53% (le chargement de 40% du système D est uniformément fractionné et ajouté à la charge de 40% du système A et du système C). Pour plusieurs raisons, cette hypothèse linéaire n’est pas parfaitement exacte, mais elle offre suffisamment de précision pour la jauge.  
  ...
  **Autre scénario:** Deux contrôleurs de domaine en cours d’exécution à 40%: Un contrôleur de domaine échoue, l’UC estimée sur le restant est estimée à 80%. Cette limite dépasse les seuils indiqués ci-dessus pour le plan de capacité et commence à limiter gravement la quantité de la salle de la tête pour les 10 à 20% visibles dans le profil de charge ci-dessus, ce qui signifie que les pics pilotent le contrôleur de disque à 90% à 100% au cours du scénario «*N*» et de baisse de la réactivité de façon limitée.
...
Pour plus d’informations, [consultez l’article 2688798 de la base de connaissances: Comment effectuer le réglage des performances pour l’authentification NTLM à l’aide](http://support.microsoft.com/kb/2688798)du paramètre MaxConcurrentApi.
...
|Stockage|Disque logique ( *\<disque de\>base de données NTDS*) \Avg disque s/lecture<br /><br />Disque logique ( *\<disque de\>base de données NTDS*) \Avg disque s/écriture|60 min|10 ms|15 ms|
|Services AD|Temps d’attente du\*sémaphore \Latence Netlogon ()|60 min|0|1 seconde|
...
- Traduction du [PERF_100NSEC_TIMER_INV](https://docs.microsoft.com/en-us/previous-versions/windows/embedded/ms901169(v=msdn.10))
  - *B* = le nombre de threads «inactifs» de 100-ns passe sur le processeur logique. Modification de la variable «*X*» dans le calcul [PERF_100NSEC_TIMER_INV](https://docs.microsoft.com/en-us/previous-versions/windows/embedded/ms901169(v=msdn.10))
  - *T* = nombre total d’intervalles de 100-ns dans un intervalle de temps donné. Modification de la variable «*Y*» dans le calcul [PERF_100NSEC_TIMER_INV](https://docs.microsoft.com/en-us/previous-versions/windows/embedded/ms901169(v=msdn.10)) .
...
## <a name="appendix-b-considerations-regarding-different-processor-speeds-and-the-effect-of-processor-power-management-on-processor-speeds"></a>Annexe B : Considérations relatives aux différentes vitesses de processeur et à l’effet de la gestion de l’alimentation du processeur sur les vitesses de processeur
...
- Par exemple, dans un scénario où 11,25 processeurs sont demandés, si les processeurs étaient exécutés à mi-vitesse lors de la collecte des données, l’estimation &divide; la plus précise peut être 5,125 2.
...
1. Pour déterminer le modificateur, utilisez l’équation suivante:
   >((*Valeur* &times; du score par cœur &divide; de la plateforme cible) (*MHz par cœur de plateforme de référence*) &times; ((*valeur du score de base par* cœur) (*MHz par cœur de plateforme cible* ))  

    À l’aide de l’exemple ci-dessus:
   >(35,83 &times; 2000) &divide; (33,75 &times; 2300) = 0,92
1. Multipliez le nombre estimé de processeurs par le modificateur.  Dans le cas ci-dessus, pour passer du processeur E5-2650 au processeur E5-2630, multipliez les &times; 11,25 UC 0,92 = 10,35 processeurs calculées nécessaires.
...
Le système d’exploitation crée une file d’attente d’e/s FIFO (premier entré/premier sorti) pour chaque disque observé; ce disque peut représenter un axe, un tableau ou une partition de tableau. Du point de vue du système d’exploitation, en ce qui concerne la gestion des e/s, plus la file d’attente est active, plus les files d’attente sont performantes. Une file d’attente FIFO est sérialisée, ce qui signifie que toutes les e/s émises vers le sous-système de stockage doivent être traitées dans l’ordre d’arrivée de la demande. En mettant en corrélation chaque disque observé par le système d’exploitation avec un axe/une baie, le système d’exploitation gère maintenant une file d’attente d’e/s pour chaque ensemble unique de disques, ce qui élimine la contention des ressources d’e/s rares sur les disques et isole les demandes d’e/s à un seul libérer. En guise d’exception, Windows Server 2008 introduit le concept de hiérarchisation des e/s, et les applications conçues pour utiliser la priorité «basse» sont classées dans cet ordre normal et reprennent un siège. Les applications qui ne sont pas spécifiquement codées pour tirer parti de la priorité «basse» par défaut «normal».
...
- **Bus PCI –** Il s’agit d’un composant souvent négligé. Dans cet exemple, il ne s’agit pas du goulot d’étranglement; Cependant, à mesure que les systèmes évoluent, cela peut devenir un goulet d’étranglement. À des fins de référence, un bus PCI 32 bits fonctionnant sur 33Mhz peut, en théorie, transférer 133 Mo/s de données. Voici l’équation:  
  ...
  Un bus PCI 64 bits de 66 MHz peut prendre en charge un maximum théorique de &divide; (64 bits 8 &times; bits par octet 66 MHz) = 528 Mo/s. en outre, tout autre périphérique (tel que la carte réseau, le deuxième contrôleur SCSI, etc.) réduira la bande passante disponible à mesure que la bande passante est partagée et que les appareils sont en concurrence pour les ressources limitées.

Après l’analyse des composants de ce sous-système de stockage, l’axe est le facteur limitatif de la quantité d’e/s qui peut être demandée et, par conséquent, la quantité de données qui peuvent transiter par le système. Plus précisément, dans un scénario de AD DS, il s’agit de 100 e/s aléatoires par seconde par incréments de 8 Ko, soit un total de 800 Ko par seconde lors de l’accès à la base de données Jet. En guise d’alternative, le débit maximal pour une pile qui est exclusivement allouée aux fichiers journaux est soumis aux limitations suivantes: 300 e/s séquentielles par seconde par incréments de 8 Ko, pour un total de 2400 Ko (2,4 Mo) par seconde.
...
|Notes|Analyse des goulots d’étranglement|Disque|Bus|Carte|Bus PCI|
|-|-|-|-|-|-|
...
> E/s *en lecture* + &times; 2 *écriture* = e/s*disque total disponible e/s consommée*  
...
Enfin, le HBA installé sur le serveur est également limité au nombre d’e/s qu’il peut traiter. En règle générale, un deuxième HBA est installé pour la redondance, mais comme avec le commutateur SAN, lors du calcul de l’e/s maximale pouvant être gérée, le débit total de *N* &ndash; 1 HBA est le niveau d’extensibilité maximal du système.
...
- **À quel taux le cache doit-il être chaud?** En partant du principe que la charge du client va maximiser le débit sur cet exemple de stockage, le cache sera chaud à une vitesse de 2400 &times; IOPS de 8 Ko par e/s. Ou environ 20 Mo/s par seconde, en chargeant environ 1 Go de base de données en mémoire vive toutes les 53 secondes.

...
Si un serveur WSUS est exécuté en mode réplica, vous ne pourrez pas effectuer cette tâche. Pour plus d’informations sur le mode réplica, voir [exécution du mode RÉPLICA WSUS](running-wsus-replica-mode.md)et [Step 1 : Préparez-vous à votre déploiement de WSUS @ no__t-0.
...
-   Vous pouvez spécifier un port personnalisé pour la synchronisation. Pour plus d’informations sur la configuration des ports, voir [Step 3 : Configurez WSUS @ no__t-0 dans le Guide de déploiement de WSUS.

-   Vous pouvez utiliser SSL (Secure Socket Layer) pour sécuriser la synchronisation des informations de mise à jour entre les serveurs WSUS. Pour plus d’informations sur l’utilisation de SSL, consultez la section «3,5. Sécurisation de WSUS avec le protocole protocole SSL» de [Step 3 : Configurez WSUS @ no__t-0 dans le Guide de déploiement de WSUS.

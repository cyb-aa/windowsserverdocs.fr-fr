...
description: 'Rubrique Windows Server Update Service (WSUS) : vue d’ensemble du processus de planification du déploiement avec des liens vers les rubriques connexes.'
...
|Tâche|Description|
|----|--------|
|[1,1. Examiner les considérations et la configuration système requise @ no__t-0|Passez en revue la liste des considérations et la configuration requise pour vous assurer de disposer de tous les matériels et logiciels nécessaires pour déployer WSUS.|
|[1,2. Choisir un scénario de déploiement WSUS @ no__t-0|Choisissez le scénario de déploiement de WSUS à utiliser.|
|[1,3. Choisir une stratégie de stockage WSUS @ no__t-0|Choisissez la stratégie de stockage WSUS la mieux adaptée à votre déploiement.|
|[1,4. Choisir les langues de mise à jour WSUS @ no__t-0|Choisissez les langues de mise à jour WSUS à installer.|
|[1,5. Planifier les groupes d’ordinateurs WSUS @ no__t-0|Planifiez l’approche des groupes d’ordinateurs WSUS à utiliser pour votre déploiement.|
|[1,6. Planifier les considérations relatives aux performances WSUS : Service de transfert intelligent en arrière-plan @ no__t-0|Planifiez une conception WSUS pour des performances optimisées.|
|[1,7. Planifier les paramètres de Mises à jour automatiques @ no__t-0|Planifiez la configuration des paramètres des Mises à jour automatiques pour votre scénario.|
...
-   La configuration matérielle requise du serveur pour activer le rôle WSUS dépend de la configuration matérielle requise. Les configurations matérielles minimales requises pour WSUS sont les suivantes :
    ...
    -   **Capacité** WSUS nécessite 2 Go supplémentaires de RAM plus que ce qui est requis par le serveur et tous les autres services ou logiciels.
...
-   Configuration logicielle requise :
    ...
    -   Pour consulter des rapports, WSUS nécessite [Microsoft Report Viewer Redistributable 2008](https://www.microsoft.com/download/details.aspx?id=6576). Sur Windows Server 2016, WSUS requiert [Microsoft Report Viewer Runtime 2012](https://www.microsoft.com/download/details.aspx?id=35747)
...
1. Les rôles de serveur qui ont eu leur charge utile supprimée à l’aide des fonctionnalités à la demande ne peuvent pas être installés à la demande à partir de Microsoft Update. Vous devez fournir une source d’installation au moment où vous essayez d’installer ces rôles de serveur, ou configurer une source pour les fonctionnalités à la demande dans stratégie de groupe.
...
3. Les appareils d’entreprise exécutant Windows 10, version 1709 ou version 1803, ne peuvent pas installer de fonctionnalités à la demande directement à partir de WSUS. Pour installer des fonctionnalités à la demande, [créez un fichier de fonctionnalité (magasin côte à côte)](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj127275%28v=ws.11%29#create-a-feature-file-or-side-by-side-store) ou obtenez le package de fonctionnalités à la demande à partir de l’une des sources suivantes :
   - [Centre](https://www.microsoft.com/licensing/servicecenter) de gestion des licences en volume (VLSC)-l’accès VL est requis
...
> [!NOTE]
> SQL Server Express 2008 R2 a une limite de taille de base de données de 10 Go. Cette taille de base de données semble suffisante pour WSUS, même si l’utilisation de cette base de données ne présente pas d’avantage notable par rapport à WID. La base de données WID a une mémoire RAM minimale requise de 2 Go au-delà de la configuration système standard de Windows Server.
...
4.  Le serveur WSUS et le serveur de base de données doivent se trouver dans le même fuseau horaire ou être synchronisés avec la même source de temps universel coordonné (heure de Greenwich).
...
1.  permet de réduire l’utilisation des liaisons WAN et améliore la réactivité des applications. Pour activer l’accélération fournie par BranchCache pour l’accès au contenu délivré le serveur WSUS, vous devez installer la fonctionnalité BranchCache sur le serveur et les clients, puis vérifier que le service BranchCache a démarré. Aucune autre étape n’est requise.
...
WSUS requiert une base de données pour chaque serveur WSUS. WSUS prend en charge l’utilisation d’une base de données qui se trouve sur un ordinateur différent du serveur WSUS, avec quelques restrictions. Pour obtenir la liste des bases de données prises en charge et des limitations de bases de données distantes, consultez la section « 1,1 Review initial Considerations and System Requirements » dans ce guide.
...
> [!NOTE]
> La base de données interne Windows (WID) a été introduite dans Windows Server 2008.
...
Cette option nécessite que le serveur dispose de suffisamment d’espace disque pour stocker toutes les mises à jour requises. au minimum, WSUS requiert 20 Go pour stocker les mises à jour localement. Toutefois, nous vous recommandons de disposer de 30 Go basés sur des variables testées.
...
> [!NOTE]
> Si un serveur WSUS est exécuté en mode Réplica, il n’est pas possible de créer des groupes d’ordinateurs sur ce serveur. Tous les groupes d’ordinateurs nécessaires pour les ordinateurs clients du serveur de réplication doivent être créés sur le serveur WSUS qui est la racine de la hiérarchie de serveurs WSUS. Pour plus d’informations sur le mode réplica, voir gérer les serveurs de réplication WSUS [gérer les serveurs](https://technet.microsoft.com/library/dd939893(WS.10).aspx) de réplication WSUS dans le guide des opérations WSUS 3,0 SP2.
...
Dans l’exemple de texte suivant du volet hiérarchie de la console Services de mise à jour, pour un serveur WSUS nommé WSUS-01, les groupes d’ordinateurs nommés ordinateurs de bureau et serveurs ont été ajoutés au groupe par défaut **tous les ordinateurs** . Les ordinateurs de bureau et les groupes de serveurs se trouvent au même niveau hiérarchique.
...
Il existe certaines zones que vous devez soigneusement planifier avant de déployer WSUS afin de pouvoir optimiser les performances. Les principaux domaines sont les suivants :
...
Cette fonctionnalité est parfois appelée « remise différentielle », car elle télécharge uniquement le Delta (différence) entre deux versions d’un fichier. Les fichiers d’installation rapide sont plus volumineux que les mises à jour distribuées aux ordinateurs clients, car ils contiennent toutes les versions possibles de chaque fichier à mettre à jour.
...
1.  Utilisez la limitation de bande passante du service de transfert intelligent en arrière-plan (BITS). Les limitations de bande passante du service BITS peuvent être contrôlées selon le créneau horaire, mais elles s’appliquent à toutes les applications qui utilisent le service BITS. Pour savoir comment contrôler la limitation du service BITS, consultez [stratégies de groupe](https://msdn.microsoft.com/library/windows/desktop/aa362844(v=vs.85).aspx).

...
|                 Modifier une adresse IP statique                 | **ipconfig /all** <br>Enregistrer les informations pertinentes ou rediriger vers un fichier texte (**ipconfig/all > ipconfig.txt**).<br>**interfaces de netsh interface ipv4 show**<br>Vérifiez qu’il existe une liste des interfaces.<br>**netsh interface ipv4 définir le nom de l’adresse \<ID à partir de la liste des interfaces\> source = adresse statique =\<IP adresse principale\> passerelle =\<adresse de passerelle\>**<br>Exécutez **ipconfig/all** pour vérifier que DHCP activé est défini sur **non**. |
|                   Définissez une adresse DNS statique.                   |   <strong>netsh interface ipv4 ajouter dnsserver nom =\<nom ou ID de la carte d’interface réseau\> adresse =\<adresse IP du serveur DNS principal\> index = 1 <br></strong>netsh interface ipv4 ajouter dnsserver nom =\<nom de serveur DNS secondaire\> adresse =\<adresse IP du serveur DNS secondaire\> index = 2\*\* <br> Répétez au besoin, pour ajouter des serveurs supplémentaires.<br>Exécutez **ipconfig/all** pour vérifier que les adresses sont corrects.   |
...
|        Administrer le système de fichiers        |  Pour obtenir une liste complète des commandes, exécutez **fsutil / ?**   |

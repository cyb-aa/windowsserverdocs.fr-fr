...
6. Dans **valeur** type  **\<de données\>lecteur\\:\>DedicatedDumpFile. sys, puis cliquez sur OK.\<**

   >[!NOTE] 
   > Remplacez \<le\> lecteur par un lecteur disposant d’un espace disque suffisant pour le fichier d' \<échange, puis\> remplacez DedicatedDumpFile. dmp par le chemin d’accès complet au fichier dédié.
...
Définissez \<filePath\> sur le chemin de destination. Par exemple, la commande suivante définit le chemin d’accès de destination de l’image mémoire sur C:\WINDOWS\MEMORY. DMP 
...
\<La\> valeur peut être 0, 1, 2 ou 3, comme indiqué ci-dessous.

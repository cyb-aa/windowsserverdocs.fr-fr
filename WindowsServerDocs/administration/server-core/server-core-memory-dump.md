...
6. Dans type de **données de valeur** **\<lecteur\>:\\\<Dedicateddumpfile. sys\>** , puis cliquez sur **OK**.

   >[!NOTE] 
   > Remplacez \<lecteur\> par un lecteur disposant d’un espace disque suffisant pour le fichier d’échange, puis remplacez \<DedicatedDumpFile. dmp\> par le chemin d’accès complet au fichier dédié.
...
Définissez \<FilePath\> sur le chemin d’accès de destination. Par exemple, la commande suivante définit le chemin d’accès de destination de l’image mémoire sur C:\WINDOWS\MEMORY. DMP 
...
\<valeur\> peut être 0, 1, 2 ou 3, comme indiqué ci-dessous.
...
- 3 : image mémoire de petite taille. Enregistre le plus petit ensemble d’informations utiles qui peuvent aider à identifier la raison pour laquelle votre ordinateur s’est arrêté de manière inattendue.
...
## <a name="step-5-configure-the-server-to-overwrite-the-existing-memory-dump-file"></a>Étape 5 : configurer le serveur pour remplacer le fichier de vidage de la mémoire existant

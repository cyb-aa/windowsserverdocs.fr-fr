...
description: Rubrique de référence pour la sous-commande Set-Server, qui a configuré les paramètres d’un serveur des services de déploiement Windows.
...
|[/AnswerClients : {All &#124; &#124; connues aucun}]|Spécifie les clients auxquels ce serveur répond. Si vous définissez cette valeur sur **connu**, un ordinateur doit être prédéfini dans les services de domaine Active directory (AD DS) avant qu’il ne soit traité par le serveur des services de déploiement Windows.|
...
|[/NewMachineNamingPolicy :<Policy>]|Spécifie le format à utiliser lors de la génération de noms d’ordinateur pour les clients. Pour plus d’informations sur le format à <policy>utiliser pour, cliquez avec le bouton droit sur le serveur dans le composant logiciel enfichable MMC, cliquez sur **Propriétés**, puis affichez l’onglet **services d’annuaire** . Par exemple, **/NewMachineNamingPolicy :% 61Username% #**.|
...
de syntaxe de ligne de commande à
l’aide[de la commande Disable-Server](using-the-disable-server-command.md)à l’aide de la commande[Enable-Server](using-the-enable-server-command.md)
[à](using-the-get-server-command.md)
l’aide de la commande-Server,[à l’aide de la commande Initialize-Server](using-the-initialize-server-command.md)

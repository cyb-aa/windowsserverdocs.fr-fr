...
|[/insert | /overstrike]|Spécifie s’il faut insérer ou remplacer le texte en cours de frappe. Si vous utilisez **/insertion**, nouveau texte que vous tapez sur une ligne est insérée dans le milieu du texte existant. Si vous utilisez **/overstrike**, le nouveau texte remplace le texte existant. Le paramètre par défaut est **/overstrike**.|
...
-   Rappel d’une commande
    ...
    |Touche|Description|
    |---|-----------|
    |Haut|Rappelle la commande que vous avez utilisé avant celle qui s’affiche.|
    |Bas|Rappelle la commande que vous avez utilisé après celle qui s’affiche.|
    |Page précédente|Rappelle la première commande que vous avez utilisé dans la session active.|
    |Page suivante|Rappelle la dernière commande que vous avez utilisé dans la session active.|
-   Modification de la ligne de commande
    ...
    |Droite|Déplace le point d’insertion d’un caractère.|
...
-   Création d’une macro
    ...
    |$B ou $b|Envoie la sortie de la macro à une commande. Ces caractères spéciaux sont équivalentes à l’aide du canal (**|*) sur une ligne de commande.|
    ...
    |$*|Représente toutes les informations de ligne de commande que vous souhaitez spécifier lorsque vous tapez le nom de macro. Le caractère spécial **$ \*** est un paramètre remplaçable semblable aux paramètres de lot **$1** via **$9**, avec un important différence : tout ce que vous tapez sur la ligne de commande une fois que le nom de macro est remplacé par le **$ \*** dans la macro.|

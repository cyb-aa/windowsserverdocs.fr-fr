...
## <a name="BKMK_Contents"></a>Matières
...
-   Pour obtenir la syntaxe d’utilisation de Certutil avec un verbe spécifique, exécutez le verbe **certreq**  *\<>* **- ?**
...
|ValidityPeriodUnits</br>Remarque : Utilisé uniquement lorsque le type de demande est CERT.|Spécifie un nombre d’unités à utiliser avec ValidityPeriod.|Numeric|ValidityPeriodUnits = 3|
|ValidityPeriod</br>Remarque : Utilisé uniquement lorsque le type de demande est CERT.|VValidityPeriod doit être une période de temps en anglais américain.|Années, mois, semaines, jours, heures, minutes, secondes|ValidityPeriod = années|
...
|-PolicyServer  *\<PolicyServer >*|« LDAP :  *\<chemin >* »</br>Insérez l’URI ou l’ID unique d’un ordinateur exécutant l’service Web Stratégie d’inscription de certificats.</br>Pour spécifier que vous souhaitez utiliser un fichier de requête en navigant, utilisez simplement un signe moins (-) pour  *\<policyserver >* .|
...
|-ClientCertificate  *\<ClientCertId >*|Vous pouvez remplacer la  *\<> ClientCertID* par une empreinte de certificat, CN, EKU, template, email, UPN et la nouvelle syntaxe name = value.|
|-UserName  *\<nom_utilisateur >*|Utilisé avec les services Web d’inscription de certificats. Vous pouvez remplacer  *\<le nom d’utilisateur >* par le nom Sam ou domaine\utilisateur. Cette option est destinée à être utilisée avec l’option-p.|
|-p  *\<mot de passe >*|Utilisé avec les services Web d’inscription de certificats. Remplacez le mot de  *passe>parlemotdepassedel’utilisateurréel\<* . Cette option est destinée à être utilisée avec l’option-UserName.|
...
|Formats|Description|
|-------|-----------|
|RequestFileIn|Nom du fichier d’entrée binaire ou encodé en base64 : Demande de certificat #10 PKCS, demande de certificat CMS, PKCS #7 demande de renouvellement de certificat, certificat X. 509 à certification croisée ou demande de certificat au format de balise KeyGen.|
...
-   [Guide pas à pas d’AD CS : Déploiement de la hiérarchie PKI à deux niveaux](https://social.technet.microsoft.com/wiki/contents/articles/15037.ad-cs-step-by-step-guide-two-tier-pki-hierarchy-deployment.aspx)

...
ESL est uniquement disponible pour le nom d’utilisateur et qui proviennent via l’extranet avec le Proxy d’Application Web ou une prise en charge les demandes d’authentification de mot de passe 3e proxy tiers.   
...
- Get-ADFSAccountActivity 
  ...
  Exemple : Get-ADFSAccountActivity user@contoso.com 
...
-    Set-ADFSAccountActivity 
     ...
     Exemple : Set-ADFSAccountActivity user@contoso.com - AdditionalFamiliarIps « 1.2.3.4 »
...
- Reset-ADFSAccountLockout 
  ...
  Réinitialise les compteurs de l’emplacement inconnu (badPwdCountUnfamiliar) ou un compteur de verrouillage d’un compte d’utilisateur pour chaque emplacement connu (badPwdCountFamiliar). En réinitialisant un compteur, la valeur « FamiliarLockout » ou « UnfamiliarLockout » met à jour, car le réinitialiser le compteur sera inférieur au seuil.  
   ...
   Exemple : Réinitialisation-ADFSAccountLockout user@contoso.com -exemple familier d’emplacement :  Réinitialisation-ADFSAccountLockout user@contoso.com -emplacement inconnu 
...
Pour les événements de verrouillage intelligent Extranet à écrire, ESL doit être activé en mode « journal uniquement » ou « appliquer » et l’audit de sécurité AD FS est activé. AD FS écrira les événements de verrouillage extranet dans le journal d’audit de sécurité : 

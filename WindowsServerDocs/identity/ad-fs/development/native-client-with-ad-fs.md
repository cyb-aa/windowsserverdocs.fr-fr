---
title: Créer une application cliente native à l’aide des clients publics OAuth avec AD FS 2016
description: Une procédure pas à pas qui fournit des instructions de la création d’une application cliente native à l’aide des clients publics OAuth et AD FS 2016
author: billmath
ms.author: billmath
ms.reviewer: anandy
manager: mtillman
...
ms.locfileid: "59846480"
---
# <a name="build-a-native-client-application-using-oauth-public-clients-with-ad-fs-2016"></a>Créer une application cliente native à l’aide des clients publics OAuth avec AD FS 2016

## <a name="overview"></a>Vue d'ensemble

Cet article explique comment créer une application native qui interagit avec une API Web protégée par AD FS 2016.
...
2. Le jeton d’accès est utilisé comme un jeton du porteur pour authentifier l’utilisateur lors de l’appel /todolist point de terminaison de l’API de web TodoListService.
 Nous être à l’aide de l’exemple d’application pour Azure AD ici et modifiez-la ensuite pour AD FS 2016.
...
* Ajouter la clé **ida : autorité** avec la valeur illustrant la publicité, exemple, de service FS https://fs.contoso.com/adfs/
* Modifier **ida : ClientId** clé avec la valeur de la **Application Native** page lors de la création du groupe d’applications dans AD FS. Pour ex, 3f07368b-6efd-4f50-a330-d93853f4c855
* Modifier le **todo:TodoListBaseAddress** à l’adresse de base de l’API Web, par exemple, https://localhost:44321/
* Définissez la valeur de **ida : RedirectUri** à la valeur que vous placez dans la page de l’Application Native pendant ajouter l’Application de groupe dans AD FS.
* Pour faciliter la lecture, vous pouvez supprimer ou commenter la clé pour **ida : client** et **ida : AADInstance**.

**MainWindow.xaml.cs**

* Supprimez la ligne pour aadInstance
...
* Dans la fonction **MainWindow**, modifiez l’instanciation d’authContext en tant que
...
* Supprimer la clé **ida : client** que nous n’en avez besoin

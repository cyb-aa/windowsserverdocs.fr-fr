...
Les informations suivantes exposent les considérations de planification initiales à prendre en compte dans le cadre d’une activation KMS (Key Management Services, service de gestion de clés) impliquant Windows Server 2016. Pour plus d’informations sur l’activation de KMS impliquant des systèmes d’exploitation plus anciens que ceux répertoriés ici, consultez [étape 1 : Passez en revue et sélectionner les méthodes d’Activation](https://technet.microsoft.com/library/jj134256(WS.11).aspx).
...
Si vous convertissez un ordinateur ayant le statut d’hôte KMS, de clé d’activation multiple (MAK) ou d’une version commerciale de Windows en un client KMS, installez la clé de configuration de client KMS applicable. Pour plus d’informations, consultez [KMS Client Setup Keys](KMSclientkeys.md). 

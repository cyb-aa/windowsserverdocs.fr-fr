...
- Les mises à niveau d’architectures 32 bits vers des architectures 64 bits ne sont pas prises en charge. Toutes les éditions de Windows Server2016 sont des éditions 64bits uniquement.
...
Pour **Windows Server 2016 Essentials**: Vous pouvez convertir la version commerciale complète en entrant une vente au détail, licence en volume ou clé OEM dans la commande **slmgr.vbs**.
...
Pour Windows Server 2016 Standard, vous pouvez convertir le système Windows Server 2016 Datacenter comme suit : À partir d’une invite de commandes avec élévation de privilèges, déterminez le nom de l’édition actuelle à l’aide de la commande **DISM /online /Get-CurrentEdition**. Pour Windows Server 2016 Standard, ce sera `ServerStandard`. Exécutez la commande **DISM /Online/Get-TargetEditions** pour obtenir l’ID de l’édition que vous pouvez mettre à niveau vers. Prenez note de cet ID de l’édition, une forme abrégée du nom de l’édition. Puis exécutez **DISM /Online / Edition :\<ID de l’édition\> /ProductKey:XXXXX-XXXXX-XXXXX-XXXXX-XXXXX /AcceptEula**, en fournissant l’ID de l’édition de votre cible et sa clé de produit commercialisé. Le serveur redémarre deux fois.

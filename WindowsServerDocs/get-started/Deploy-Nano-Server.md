...
- Activer le débogage sur les protocoles IEEE1394 ou série, USB, TCP/IP
...
|Pilotes de base pour plusieurs cartes réseau et contrôleurs de stockage. Il s’agit du même jeu de pilotes inclus dans une installation minimale de WindowsServer2016.|-OEMDrivers|
...
|Internet Information Server (IIS)|-Package Microsoft-NanoServer-IIS-Package<br />**Remarque:** Pour plus d’informations sur l’utilisation d’IIS, consultez [IIS sur Nano Server](IIS-on-Nano-Server.md).|
...
|Agent System Center Operations Manager| Installé séparément. Consultez la documentation de System Center Operations Manager pour plus d’informations à https://technet.microsoft.com/system-center-docs/om/manage/install-agent-on-nano-server.|
...
|Déploiement sur une machine physique|-Package Microsoft-NanoServer-Host-Package|
...
### Éléments de clustering de basculement installés avec le paramètre -Clustering
...
### Éléments de stockage et de fichier installés avec le paramètre -Storage
...
### Installation d’un fichier VHD Nano Server  
...
3.  Copier le support d’installation à partir de \\\Path\To\Media\server_en-us into .\Base  
...
### Installation d’un fichier WIM Nano Server  
  
1.  Copiez le dossier *NanoServerImageGenerator* du dossier \NanoServer du fichier ISO de WindowsServer2016 dans un dossier local sur votre ordinateur.  
...
Où  
-   -MediaPath correspond à la racine du support DVD ou à l’image ISO contenant WindowsServer2016.  
...
### Modification des fichiers sur Nano Server localement et à distance  
...
## <a name="BKMK_online"></a>Installation des rôles et des fonctionnalités en ligne  
> [!NOTE]
...
### Installation des rôles et des fonctionnalités à partir d’un référentiel de packages  
...
### Recherche des packages Nano Server  
...
### Installation de packages Nano Server  
...
### Téléchargement de packages Nano Server  

`Save-NanoServerPackage` ou `Save-Package` vous permettent de télécharger des packages et de les enregistrer sans les installer. Ces deux applets de commande acceptent des entrées provenant du pipeline.
...
`Save-Package -ProviderName NanoServerPackage -Name Microsoft-NanoServer-IIS-Package -Path C:\ -Culture it-IT -MinimumVersion 10.0.14393.0` enregistre une version particulière et pour la langue et les paramètres régionaux italiens.
...
### Installation des rôles et des fonctionnalités à partir d’une source locale  
...
4.  Accédez au répertoire contenant le fichierXML créé et exécutez  
...
5.  Vérifiez que le package et son module linguistique sont correctement installés en exécutant:  
    ...
    **dism /online /get-packages**  
    ...
    «Identité du package: Microsoft-NanoServer-IIS-Package~31bf3856ad364e35~amd64~en-US~10.0.10586.0» devrait être répertorié deux fois, une fois pour Type de version: Module linguistique et une fois pour Type de version: Feature Pack.  
  
## Personnalisation d’un fichier VHD Nano Server existant  
...
## Tâches supplémentaires que vous pouvez accomplir avec New-NanoServerImage et Edit-NanoServerImage  
  
### Jonction de domaines  
...
1.  Installez WindowsServer2016 sur l’ordinateur physique sur lequel vous exécuterez Nano Server.
...
> [!NOTE]  
> Dans le dossier dans lequel se trouvent vos pilotes, les fichiers SYS et les fichiers INF correspondants doivent être présents. En outre, Nano Server prend uniquement en charge les pilotes signés 64bits. 
...
### Définition des adresses IP statiques  
...
### Exécution de commandes personnalisées après le premier démarrage
...
### Exécution de scripts PowerShell personnalisés dans le cadre du processus de création d’image
...
### Prise en charge des scénarios de développement
...
### Fichier d’installation sans assistance personnalisé  
...
> [!NOTE]
> NANOServer ne prend pas en charge la définition des paramètres TCP/IP via des fichiers d’installation sans assistance. Vous pouvez utiliser Setupcomplete.cmd pour configurer les paramètres TCP/IP.
...
1. Installez le [kit de développement logiciel (SDK) Windows10](https://developer.microsoft.com/windows/downloads/windows-10-sdk), qui inclut les outils nécessaires pour créer un package WSA: MakeAppx, MakeCert, Pvk2Pfx, SignTool.
2. Déclarez un manifeste: suivez le [schéma d’extension de manifeste WSA](https://msdn.microsoft.com/library/windows/apps/mt670653.aspx) pour créer le fichier de manifeste, AppxManifest.xml.
...
1. Exécutez l’applet de commande PowerShell [*Import-Certificate*](https://technet.microsoft.com/library/hh848630) pour importer le certificat de l’éditeur de l’étape4 ci-dessus dans Nano Server avec certStoreLocation à l’emplacement «Cert: \LocalMachine\TrustedPeople». Par exemple: `Import-Certificate -FilePath ".\xyz.cer" -CertStoreLocation "Cert:\LocalMachine\TrustedPeople"`
...
#### Ressources supplémentaires pour créer des applications
WSA est l’extension de serveur de la technologie de package d’application Windows (même si elle n’est pas hébergée dans le MicrosoftStore). Si vous souhaitez publier des applications avec WSA, ces rubriques vous aideront à vous familiariser avec le pipeline de package d’application:

- [Comment créer un manifeste de package de base pour Windows8](https://msdn.microsoft.com/library/windows/desktop/br211475.aspx)
- [Application Packager (MakeAppx.exe) (Outil de création de package d’application (MakeAppx.exe))](https://msdn.microsoft.com/library/windows/desktop/hh446767(v=vs.85).aspx)
- [How to create an app package signing certificate (Comment créer un certificat de signature de package d’application)](https://msdn.microsoft.com/library/windows/desktop/jj835832(v=vs.85).aspx)
- [SignTool (SignTool)](https://msdn.microsoft.com/library/windows/desktop/aa387764(v=vs.85).aspx)

### Installation de pilotes sur Nano Server
...
- [Driver Signing (Signature du pilote)](https://msdn.microsoft.com/windows/hardware/drivers/install/driver-signing)
- [Using a Universal INF File (Utilisation d’un fichier INF universel)](https://msdn.microsoft.com/windows/hardware/drivers/install/using-a-configurable-inf-file)

#### Installation des packages de pilotes hors connexion
...
#### Installation des packages de pilotes en ligne
...
### Pour ajouter Nano Server à un domaine en ligne  
...
7. Pour des raisons de sécurité, retirez Nano Server de la liste des hôtes approuvés avec cette commande:
`Set-Item WSMan:\localhost\client\TrustedHosts ""` 
  
**Autre méthode de jonction de domaine en une seule étape**  
...
Créez un dossier «Panther» (utilisé par les systèmes Windows pour stocker les fichiers pendant l’installation; consultez [Windows 7, WindowsServer 2008 R2, and WindowsVista setup log file locations](https://support.microsoft.com/en-us/kb/927521) (Emplacements du fichier journal d’installation de Windows7, WindowsServer2008R2 et WindowsVista) si vous cela vous intéresse), copiez le fichier Unattend.xml dedans, puis désinstallez le VHD avec les commandes suivantes:  
...
## Utilisation des rôles serveur sur Nano Server

###Utilisation d’Hyper-V sur Nano Server  
Hyper-V fonctionne de la même manière sur Nano Server que sur WindowsServer en mode Server Core, à deux exceptions près:  
  
-   Vous devez effectuer toute l’administration à distance et l’ordinateur d’administration doit exécuter la même version de WindowsServer que le Nano Server. Les versions antérieures des applets de commande Windows PowerShell Hyper-V ou de Gestionnaire Hyper-V ne fonctionnent pas.  
...
-   Création d’ordinateurs virtuels de génération1 et de génération 2  
...
-   [Enabling Hyper-V Remote Management - Configuring Constrained Delegation For SMB and Highly Available SMB (Activation de la gestion à distance Hyper-V - Configuration de la délégation contrainte pour SMB et SMB à haute disponibilité)](http://blogs.msdn.com/b/taylorb/archive/2012/03/20/enabling-hyper-v-remote-management-configuring-constrained-delegation-for-smb-and-highly-available-smb.aspx)  
  
-   [Enabling Hyper-V Remote Management - Configuring Constrained Delegation For Non-Clustered Live Migration (Activation de la gestion à distance Hyper-V - Configuration de la délégation contrainte pour la migration dynamique pas en cluster)](http://blogs.msdn.com/b/taylorb/archive/2012/03/20/enabling-hyper-v-remote-management-configuring-constrained-delegation-for-non-clustered-live-migration.aspx)  
...
### <a name="BKMK_Failover"></a>Utilisation du clustering de basculement sur Nano Server  
...
-   Tous les nœuds de cluster de Nano Server doivent être joints au même domaine, similaire aux nœuds de cluster dans WindowsServer.  
  
-   Le compte de domaine doit disposer des privilèges d’administrateur sur tous les nœuds Nano Server, comme avec les nœuds de cluster dans WindowsServer.  
...
Vous pouvez créer un cluster avec `New-Cluster -Name <clustername> -Node <comma-separated cluster node list>`  
...
Ajoutez un nœud supplémentaire au cluster avec `Add-ClusterNode -Name <comma-separated cluster node list>  -Cluster <clustername>`  
  
Supprimez un nœud du cluster avec  `Remove-ClusterNode -Name <comma-separated cluster node list>  -Cluster <clustername>`  
  
Créez un serveur de fichiers avec montée en puissance parallèle avec `Add-ClusterScaleoutFileServerRole -name <sofsname> -cluster <clustername>`  
...
### <a name="BKMK_DNS"></a>Utilisation du serveur DNS sur Nano Server  
...
### <a name="BKMK_IIS"></a>Utilisation d’IIS sur Nano Server  
...
### Utilisation de MPIO sur Nano Server
Pour savoir comment utiliser MPIO, consultez [MPIO sur Nano Server](MPIO-on-Nano-Server.md). 

### <a name="BKMK_SSH"></a>Utilisation de SSH sur Nano Server
...
## Annexe: exemple de fichier Unattend.xml qui joint Nano Server à un domaine  

...
Au-delà des scénarios de déploiement fournis, vous pouvez ajouter d’autres fonctionnalités VPN avancées pour améliorer la sécurité et la disponibilité de votre connexion VPN. Par exemple, ces composants permettent de s’assurer que le client qui se connecte est sain avant d’autoriser une connexion.
...
|Option  |Description  |
|---------|---------|
|Résilience de serveur et équilibrage de charge     |Dans les environnements nécessitant une haute disponibilité ou prenant en charge un grand nombre de requêtes, vous pouvez augmenter les performances et la résilience de l’accès à distance en utilisant l’équilibrage de charge entre plusieurs serveurs qui exécutent le serveur NPS (Network Policy Server) et l’activation de l’accès à distance Accès au cluster de serveurs.<p>Documents associés:<ul><li>[Équilibrage de charge du serveur proxy NPS](../../../../../networking/technologies/nps/nps-manage-proxy-lb.md)</li><li>[Déployer l’accès à distance dans un cluster](https://docs.microsoft.com/windows-server/remote/remote-access/ras/cluster/deploy-remote-access-in-cluster)</li></ul>        |
...
|Option  |Description  |
|---------|---------|
|Filtrage du trafic     |Si vous devez appliquer les applications auxquelles les clients VPN peuvent accéder, vous pouvez activer les filtres de trafic VPN.<p>Pour plus d’informations, consultez [fonctionnalités de sécurité VPN](https://docs.microsoft.com/windows/access-protection/vpn/vpn-security-features).         |
...
|Accès conditionnel VPN   |L’accès conditionnel et la conformité des appareils peuvent exiger que les appareils gérés respectent les normes avant de pouvoir se connecter au VPN. L’une des fonctionnalités avancées de l’accès conditionnel VPN vous permet de limiter les connexions VPN à celles où le certificat d’authentification client contient l’OID «AAD ConditionalAttribute Access» de «1.3.6.1.4.1.311.87».<p>Pour restreindre les connexions VPN, vous devez:<ol><li>Sur le serveur NPS, ouvrez le composant logiciel enfichable **serveur de stratégie réseau** .</li><li>Développez **stratégies** > **réseau**stratégies.</li><li>Cliquez avec le bouton droit sur la stratégie réseau des **connexions de réseau privé virtuel (VPN)** et sélectionnez **Propriétés**.</li><li>Sélectionnez l’onglet **paramètres** .</li><li>Sélectionnez **spécifique au fournisseur** et sélectionnez **Ajouter**.</li><li>Sélectionnez l’option **allowed-Certificate-OID** , puis sélectionnez **Ajouter**.</li><li>Collez l’OID d’accès conditionnel AAD de **1.3.6.1.4.1.311.87** en tant que valeur d’attribut, puis cliquez deux fois sur **OK** .</li><li>Sélectionnez **Fermer** , puis **appliquer**.<p>Désormais, lorsque des clients VPN tentent de se connecter à l’aide d’un certificat autre que le certificat Cloud à courte durée de vie, la connexion échoue.</li></ol>Pour plus d’informations sur l’accès conditionnel, consultez [VPN et accès conditionnel](https://docs.microsoft.com/windows/access-protection/vpn/vpn-conditional-access).   |

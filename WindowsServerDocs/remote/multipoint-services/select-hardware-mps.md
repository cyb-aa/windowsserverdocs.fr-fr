...
Les types de processeurs suivants ont été testés pour être utilisés sur un système multipoint services\-avec des programmes de traitement à grande demande, tels que des programmes multimédias :  
  
-   **Processeur\-double cœur :** Peut prendre en charge jusqu’à 8 stations.  
-   **Processeur\-quadruple cœur :** Peut prendre en charge jusqu’à 16 stations.
-   **Processeur\-quadruple cœur avec multithread :** Peut prendre en charge jusqu’à 20 stations.      
-   **Processeur\-six cœurs avec multithread :** Peut prendre en charge jusqu’à 24 stations.  
...
Une fois que vous avez sélectionné les composants matériels de votre système multipoint services, assurez-vous d’obtenir\-les pilotes 64 bits actuels des composants.  
...
[Sélection de\-périphériques\-de station de client USB ou à connexion directe](#BKMK_Selectingdirect-video-connectedorUSBzeroclientstationdevices)   
[Sélection d’autres périphériques de station](#selecting-other-station-peripheral-devices)  
[Sélection du\-matériel\-de\-station connectée à RDP sur LAN](#BKMK_SelectingRDP-over-LAN-connectedstationhardware)  
[Sélection de périphériques audio](#selecting-audio-devices)  
  
## <a name="selecting-video-hardware"></a>Sélection du matériel vidéo
Le matériel vidéo que vous sélectionnez doit prendre en charge le nombre d’analyses dont vous aurez besoin pour le nombre d’utilisateurs que vous avez l’intention de travailler sur les stations MultiPoint services. En outre, différents types de matériel vidéo peuvent fournir une solution\-de performances plus élevée\-pour les programmes gourmands en graphiques, tels que le contenu multimédia.  
...
Une carte vidéo interne est branchée\-sur la carte mère de l’ordinateur. La carte vidéo interne est une solution qui peut aider à améliorer les performances\-des programmes multimédias riches en graphiques. Toutefois, une carte vidéo interne requiert un emplacement PCI ou PCIe disponible pour la\-connexion à la carte mère. De nombreuses\-cartes vidéo haute performance nécessitent un emplacement PCIe, mais il existe un nombre limité d’emplacements PCIe sur une carte mère. Vous devez savoir quels sont les types d’emplacements de carte vidéo disponibles sur votre ordinateur afin que vous puissiez acheter le type de carte vidéo approprié.  
...
Sélectionnez un client USB zéro si vous souhaitez activer la prise en charge d’autres analyses sans ouvrir l’ordinateur, ou si vous souhaitez prendre en charge plus de stations que de sorties vidéo disponibles. Par exemple, si vous aviez précédemment quatre moniteurs branchés\-sur des cartes vidéo internes et que vous souhaitez ajouter deux moniteurs supplémentaires, vous pouvez brancher\-deux contrôleurs vidéo externes sur l’ordinateur et disposer d’un espace pour deux moniteurs supplémentaires. De cette manière, vous pouvez combiner un client Zero USB avec le contrôleur vidéo et ne pas utiliser d’emplacements PCI ou PCIe supplémentaires sur la carte mère.  
  
## <a name="BKMK_Selectingdirect-video-connectedorUSBzeroclientstationdevices"></a>Sélection de\-périphériques\-de station de client USB ou à connexion directe  
Une station multipoint services se compose d’un concentrateur de station ou d’un client USB zéro avec\-un clavier et une souris branchés,\-et d’une analyse qui est connectée à l’ordinateur hôte ou à un client USB Zero. Les autres périphériques peuvent être branchés\-au concentrateur de station ou au client USB zéro, mais ils ne sont pas requis pour créer une station multipoint. Ces autres périphériques sont décrits dans la rubrique [sélection d’autres périphériques de station](#selecting-other-station-peripheral-devices).  
...
Une considération supplémentaire est l’utilisation d’un concentrateur alimenté de manière *externe* , au lieu d’un concentrateur *alimenté par bus\-* . Avec un concentrateur alimenté par bus\-, la quantité de courant fournie par l’ordinateur hôte doit être suffisante pour fournir de la puissance à tous les périphériques connectés\-au concentrateur, sans dégrader les performances du système. Un concentrateur alimenté en externe vous permet de connecter un plus grand nombre de périphériques et de fournir une alimentation suffisante à tous. L’utilisation de hubs alimentés en externe permet d’éviter les problèmes de performances, les défaillances de port et d’autres problèmes intermittents.  
...
||Mue|  
|-|-----------|  
|Concentrateur de station|Peut être alimenté\-par bus,\-à moins que des appareils\-de grande alimentation ne soient branchés à celui-ci ou qu’un hub en aval soit connecté à celui-ci.|  
|Hub intermédiaire |Doit être alimenté en externe|  
|Hub en aval|Peut être alimenté en externe ou alimenté par bus en fonction des appareils connectés\-au hub|  
...
Un client USB à zéro est un concentrateur USB qui contient une sortie vidéo. Par conséquent, elle permet à une analyse d’être connectée à l’ordinateur via une connexion USB. Pour plus d’informations sur l’utilisation de clients USB nuls pour la vidéo, consultez [sélection de matériel vidéo](#selecting-video-hardware) dans ce document. Un client USB à zéro peut également activer la connexion de divers périphériques USB et non\-USB au concentrateur. Les clients USB nuls sont générés par des fabricants de matériel spécifiques et nécessitent l'\-installation d’un pilote spécifique à l’appareil.  
  
### <a name="selecting-keyboards-and-mouse-devices"></a>Sélection des claviers et des périphériques de souris  
Les périphériques clavier et souris que vous connectez\-à la station sont généralement des périphériques USB. Certains clients USB à zéro fournissent\/des ports PS 2. dans ce cas, le clavier et la souris\/doivent utiliser PS 2 pour se connecter au concentrateur de station. Vous pouvez également utiliser un clavier\/et une souris PS 2 Si vous configurez une\/station connectée\-à une vidéo directe\-PS 2.  
...
Une seule analyse doit être fournie pour chaque station multipoint services, sauf si un\-écran fractionné est planifié. Les moniteurs sont insérés dans la carte vidéo de l’ordinateur, le client USB Zero ou\-le client LAN. Tout type d’analyse pris en charge par la carte vidéo, le client USB Zero ou le\-client LAN peut être utilisé, y compris les moniteurs CRT.  
  
Certaines analyses spéciales incluent un client LAN\-interne ou un client USB Zero. Ces moniteurs incluent généralement des jacks\/de sortie d’entrée audio et des concentrateurs USB internes pour connecter des claviers et des souris. Ils se connectent au serveur via une connexion USB ou LAN.  
...
Si un moniteur d’affichage doit être partagé par deux stations comme des stations\-d’écran fractionnées, la configuration minimale requise pour l’affichage est 1024 x 768, afin que les zones de l’écran de la station individuelle résultante soient au moins 512 x 768. Pour optimiser l’expérience\-utilisateur de l’écran de fractionnement, un écran étendu avec un minimum de résolution de 1600 x 900 est recommandé.  
...
> [!IMPORTANT]  
> Un clavier ne peut pas être connecté à un \(Hub en aval, par exemple, un concentrateur connecté à\)un concentrateur de station. Si vous connectez un clavier à un hub en aval, tous les périphériques connectés\-au hub en aval ne seront plus disponibles pour cette station. Ce comportement permet la prise en charge\-de concentrateurs de station connectés en cascade.  
  
**Disponible pour toutes les stations** Un périphérique USB qui est connecté à l’ordinateur \(par exemple, à l’aide d’un\) concentrateur de station, est disponible pour toutes les stations. En fonction de l’appareil, il peut être utilisé par plusieurs utilisateurs à la fois, ou un seul utilisateur peut y accéder à la fois. Le tableau suivant explique comment accéder aux périphériques USB.  
...
|-|------------------------------|----------------------------------------------|  
|Clavier|Non fonctionnel, sauf s’il fait partie d’une station PS/2. |Disponible pour une station individuelle<br /><br />Ne peut pas être connecté à un hub en aval|  
...
## <a name="BKMK_SelectingRDP-over-LAN-connectedstationhardware"></a>Sélection du\-matériel\-de\-station connectée à RDP sur LAN  
...
Il est important de s’assurer que lorsque vous sélectionnez des périphériques audio, ceux-ci peuvent être branchés dans le concentrateur de station, le client USB Zero ou le client LAN. Certains concentrateurs USB, les clients USB à zéro et les clients LAN disposent d’une prise jack audio analogique qui peut être utilisée \(avec les périphériques audio analogiques traditionnels tels que les écouteurs ou les earbuds.\) Les concentrateurs de station qui n’ont pas de prises analogiques peuvent utiliser des périphériques audio USB.  
  
Si vous avez configuré une station\/connectée\-à\-une vidéo directe PS 2\/à l’aide des ports PS 2 sur la carte mère de l’ordinateur pour le clavier et la souris, vous devez utiliser le son analogique sur la carte mère de l’ordinateur dans commande que le périphérique audio soit disponible pour cette station lorsque le système MultiPoint services s’exécute en mode station.  
  
Si vous n’avez pas de station\/connectée\-à\-une vidéo directe PS 2, le périphérique audio hôte sur la carte mère du système est disponible uniquement lorsque le système multipoint services s’exécute en mode console.  